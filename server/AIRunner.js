...
const AIRunner = class {
   ...
   * @param {*} type - Corresponds to the type AI. 1 = Random,
   *                    2 = Deterministic, 3 = Learning
   */
    constructor(id, type) {
        this._id = id;
    ...
    /**
   * Start the AI in another process.
   * All output to stdout is added to this._output.
   *
   * @modify {*} this._output - Changed to include the output of AI file.
   */
    startAI() {
        const inputBoard = "./AI/Board" + this._id.toString() + ".txt";
        const AI = spawn("./AI/t.exe",
            [inputBoard, "./AI/out.txt", this._type, "5", "false", 50]);
   ...
   */
    startAISync() {
        const inputBoard = "./AI/Board" + this._id.toString() + ".txt";
        const AI = spawnSync("./AI/t.exe",
            [inputBoard, "out.txt", this._type, "5", "false", 50]);
   ...
   * @return {{x: Int, y: Int}} Returns the guess made by the AI
   */
    retrieveGuess() {
        const guess = {x: parseInt(this._output[0]),
            y: parseInt(this._output[2])};
   ...
   *
   * @return {bool} Returns if it is possible to retrieve a guess
   */
    checkGuess() {
